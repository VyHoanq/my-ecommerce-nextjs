...
export default function SalesInvoice({ order }) {
  ...
  const invoieDate = convertIsoDateToNormal(order.createdAt);
  ...
  return (
    <div className="flex flex-col" >
      ...
      <div className="flex items-end justify-end mb-8">
        ...
        <button onClick={() => router.back()} className='inline-flex items-center justify-center px-4 py-3 text-xs font-bold text-gray-900 transition-all duration-200 bg-gray-100 border border-transparent rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 hover:bg-gray-200'>
          <X className="w-4 h-4"/>
        </button>
      </div>
      {/* {INVOICE} */}
      <div ref={invoiceRef}>
        <div className="max-w-4xl mx-auto border border-gray-500 p-8 rounded-sm text-black bg-white">
          {/* Header */}
          <div className="flex justify-between border-b border-gray-500 pb-8">
            ...
            <Image src={logo} alt="" className="w-36 h-16" />
          </div>
          {/* Header End */}
            ...
            <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
              ...
              <tbody>
                {order.orderItems.map((item, i) => {
                  const itemSubtotal = (item.quantity * item.price).toFixed(0)
                  return (
